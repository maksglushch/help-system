...
def user_profile(name):
    ...
    if user.role == 'volunteer':
        ...
        return render_template('needy.html', needy=user, announcements=announcements, review_form=review_form)
...
@bp.route('/needy_complete_request/<int:announcement_id>', methods=['POST'])
@login_required
def needy_complete_request(announcement_id):
    ann = db.session.get(Announcement, announcement_id)
    
    if not ann or ann.needy_id != current_user.id:
        flash('Це не ваша заявка.')
        return redirect(url_for('main.user_profile', name=current_user.name))

    ann.status = 'completed'
    
    # Якщо був волонтер, зберігаємо відгук з модалки
    if ann.volunteer_id:
        form = ReviewForm()
        if form.validate_on_submit():
        ...
        # Якщо волонтера не було
        flash('Заявку успішно закрито.')
